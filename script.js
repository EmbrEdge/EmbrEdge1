...
window.addEventListener('error', (e) => {
  try { console.error('Window error:', e && (e.message || e)); } catch (err) {}
});
...
function hideLoadingScreen() {
  try {
    if (!loadingScreen) { console.warn('hideLoadingScreen: no loadingScreen element'); return; }
    if (loadingScreen.classList.contains('loading-screen--hidden')) return;
    console.log('hideLoadingScreen: hiding');
    loadingScreen.classList.add('loading-screen--hidden');
    loadingScreen.style.pointerEvents = 'none';
    loadingScreen.style.opacity = '0';
    loadingScreen.style.visibility = 'hidden';
    setTimeout(() => {
      try { if (loadingScreen.parentNode) loadingScreen.parentNode.removeChild(loadingScreen); } catch (err) { console.warn('remove error', err); }
    }, 500);
  } catch (err) {
    console.error('hideLoadingScreen error', err);
  }
}

function attachLoadingHandlers() {
  try {
    console.log('attachLoadingHandlers, readyState=', document.readyState);
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
      hideLoadingScreen();
      return;
    }
    document.addEventListener('DOMContentLoaded', () => { console.log('DOMContentLoaded fired'); hideLoadingScreen(); });
    window.addEventListener('load', () => { console.log('window.load fired'); hideLoadingScreen(); });
    // final safety fallback
    setTimeout(() => { console.log('safety timeout fired'); hideLoadingScreen(); }, 3000);
  } catch (err) { console.error('attachLoadingHandlers error', err); }
}
